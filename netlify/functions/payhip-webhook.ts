...
const PRODUCT_MAP: {
  [key: string]: { packId: string; coins: number; bonus: number };
} = {
...
export const handler: Handler = async (event) => {
  ...
  if (!secret) {
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "payhip_secret_not_configured" }),
    };
  }
  ...
  const headerSig = (event.headers["x-payhip-signature"] ||
    event.headers["X-Payhip-Signature"] ||
    "") as string;
  let verified = false;
  ...
  for (const it of items) {
    const permalink =
      it.product_permalink ||
      it.product_permalink_full ||
      it.product_permalink_url ||
      it.product_permalink ||
      "";
    const permalinkNormalized = permalink.split("?")[0];
    const mapping =
      PRODUCT_MAP[permalinkNormalized] || PRODUCT_MAP[it.product_permalink];
    if (mapping) {
      const credits =
        mapping.coins +
        Math.round((mapping.coins * (mapping.bonus || 0)) / 100);
      ...
      if (permalinkNormalized) {
        for (const k of Object.keys(PRODUCT_MAP)) {
          if (
            k.includes(permalinkNormalized) ||
            permalinkNormalized.includes(k)
          ) {
            const m = PRODUCT_MAP[k];
            const credits =
              m.coins + Math.round((m.coins * (m.bonus || 0)) / 100);
            totalCredits += credits * Number(it.quantity || 1);
  ...
  try {
    if (buyerUid) {
      ...
      await db
        ...
        .doc(buyerUid)
        .set(
          { balances: { available: FieldValue.increment(totalCredits) } },
          { merge: true },
        );
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ ok: true, credits: totalCredits }),
    };
  } catch (e: any) {
